(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.EventsClientLib = {}));
    ...
    class EventsClient {
        ...
        onAck(handler) {
            if (!this.hub)
                throw new Error('Hub not initialized. Call connect() first.');
            this.hub.on('ReceiveAcknowledgement', handler);
        }
        async sendAck(requestId, message = 'Processed successfully') {
            if (!this.hub)
                throw new Error('Hub not initialized. Call connect() first.');
            if (this.hub.state !== 'Connected') {
                throw new Error(`Hub is not connected (state=${this.hub.state}). Call connect() and await it before sending acknowledgements.`);
            }
            try {
                // Prefer invoke when expecting a server result, otherwise use send
                await this.hub.invoke('SendAcknowledgement', requestId, message);
                return true;
            }
            catch (err) {
        ...
        onEvent(handler) {
            if (!this.hub)
                throw new Error('Hub not initialized. Call connect() first.');
            this.hub.on('ReceiveEvent', handler);
        }
    }
